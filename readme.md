...
**Implementation note:** The reward tracker (`browser_env/reward.py`) currently implements the milestone-based subset: target page detection (+0.3), per-field fill detection via `form_data` keys (+0.1 each), CAPTCHA penalty (−0.05), and success bonus (+1.0). These are currently brittle hard coded rules, and break easily on different webpages. I am looking towards migrating to RLAIF for better generalization, using LLM-as-judge with the same fundamental criteria.

 The behavioral penalties (timing, click centering, efficiency) are designed for Phase 3 RL and will be added when the GRPO training loop is built.
